...
const ProductDetailsClient: React.FC<ProductDetailsClientProps> = ({ product }) => {
  // Manage the cart state on the client side
  const [cart, setCart] = useState<any[]>([]);

  const handleAddToCart = () => {
    const updatedCart = [...cart, product];
    setCart(updatedCart);
    localStorage.setItem("cart", JSON.stringify(updatedCart)); // Store in localStorage for persistence
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-1">
            <button
              ...
              className="w-full py-2 bg-blue-600 text-white rounded-lg text-lg font-semibold transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              Add to Cart
            </button>
            <Link href={"/cart"}>
            <button
            className="w-1/2 py-2 mt-2 bg-green-600 text-white rounded-lg text-lg font-semibold transition hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              CheckOut Your Cart
            </button>
            </Link>
